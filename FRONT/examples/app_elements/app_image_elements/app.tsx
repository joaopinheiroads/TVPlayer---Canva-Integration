...
export const App = () => {
  ...
  async (operation: Operation) => {
    ...
    if (!image) throw new Error(`Unknown image ID: ${state.data.imageId}`);

    try {
      // 1. Upload normal para o Canva
      if (!image.imageRef) {
      ...
      // 2. ✅ Envia metadados separadamente para o seu controller
      const formData = new FormData();
      formData.append("imageId", state.data.imageId);
      formData.append("width", String(state.data.width));
      formData.append("height", String(state.data.height));
      formData.append("rotation", String(state.data.rotation));
      ...
      // 3. Adiciona/atualiza o elemento no design
      if (operation === Operation.Add) {
        await appElementClient.addElement({ data: state.data });
      } else {
        if (!state.update) throw new Error("Update function is not available");
        await state.update({ data: state.data });
      }
    } finally {
      setLoading(false);
    }
